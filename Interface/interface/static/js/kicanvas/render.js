...
export class Renderer {
    ...
    fit_to_bbox(bb) {
        ...
        let direction = "width";
        let scale = 0.98*Math.min(w_scale, h_scale);
        if (w_scale > h_scale) {
            direction = "height";
        }
        ...
        if (direction == "width") {
            this.ctx.translate(10-bb.x*w_scale, canvas_rect.height/2 - (bb.h/2 + bb.y)*w_scale);
        } else {
            this.ctx.translate(canvas_rect.width/2 - (bb.w/2 + bb.x)*h_scale, 10-bb.y*h_scale);
        }
    ...
    interactive_bboxes(v) {
        ...
        for (const g of v.iter_graphics()) {
            if ([items.Wire, items.LibrarySymbol].includes(g.constructor)) {
                if (g.lib_symbol != undefined && !g.lib_symbol.power) {
                    continue;
                }
    ...
    draw_Wire(w) {
        this.ctx.strokeStyle = w.stroke.color;
        this.ctx.lineWidth = w.stroke.width;
        this.ctx.beginPath();
    ...
    draw_LibrarySymbol(s) {
        ...
        for (const g of s.graphics) {
            ...
            // Check if s.id starts with power
            if (["+5V", "GND"].some(l => s.id.startsWith(l))) {
